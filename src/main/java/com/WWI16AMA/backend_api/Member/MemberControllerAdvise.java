...
public class MemberControllerAdvise {
    ...
    protected ResponseEntity<ErrorInfo> handleBadRequest(HttpServletRequest req, Exception ex) {
        return new ResponseEntity<ErrorInfo>(new ErrorInfo(req.getRequestURI().toString()+"?"+req.getQueryString(), ex), HttpStatus.BAD_REQUEST);
    }


    @ExceptionHandler(EntryNotFoundException.class)
    protected ResponseEntity<ErrorInfo> handleNoEntryFound(HttpServletRequest req, Exception ex) {
        return new ResponseEntity<ErrorInfo>(new ErrorInfo(req.getRequestURL().toString() + "?" + req.getQueryString(), ex, ex.getCause()), HttpStatus.NOT_FOUND);
    }
